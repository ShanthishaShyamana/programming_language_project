...
def generate_control_structure(root, i):
    ...
    if (root.type == "lambda"):
        ...
        if (left_child.type == ","):
            ...
            x = []
            for child in left_child.child:
                x.append(child.value)
        ...
        temp = Delta(count)
        control_structures[i].append(temp)
        generate_control_structure(root.child[1], count)
        count += 1
        temp = Delta(count)
        control_structures[i].append(temp)
        generate_control_structure(root.child[2], count)
        control_structures[i].append("beta")
        generate_control_structure(root.child[0], i)
